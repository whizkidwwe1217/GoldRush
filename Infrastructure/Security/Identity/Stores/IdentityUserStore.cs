...
namespace GoldRush.Infrastructure.Security.Identity.Stores
{
    ...
    /// <typeparam name="Role">The type representing a role.</typeparam>
    /// <typeparam name="AppIdentityDbContext">The type of the data context class used to access the store.</typeparam>
    ...
    /// <typeparam name="UserRole">The type representing a user role.</typeparam>
    ...
    /// <typeparam name="RoleClaim">The type representing a role claim.</typeparam>
    public class IdentityUserStore : IdentityUserStoreBase
    {
        /// <summary>
        ...
        public IdentityUserStore(DbContext context, IdentityErrorDescriber describer = null) : base(describer ?? new IdentityErrorDescriber())
        {
            if (context == null)
            {
        ...
        public async override Task<IdentityResult> UpdateAsync(User user, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            Context.Attach(user);
            user.ConcurrencyStamp = Guid.NewGuid().ToByteArray();
            Context.Update(user);
            try
            {
                await SaveChangesAsync(cancellationToken);
        ...
        /// <param name="normalizedUserName">The normalized user name to search for.</param>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/> used to propagate notifications that the operation should be canceled.</param>
        /// <param name="companyId">The id of the company to which this user belongs to.</param>
        /// <param name="tenantId">The id of the tenant to which this user belongs to.</param>
        /// <returns>
        /// The <see cref="Task"/> that represents the asynchronous operation, containing the user matching the specified <paramref name="normalizedUserName"/> if it exists.
        /// </returns>
        public override Task<User> FindByNameAsync(string normalizedUserName, Guid? companyId, Guid? tenantId, CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            return Users.FirstOrDefaultAsync(u => u.NormalizedUserName == normalizedUserName && u.CompanyId == companyId && u.TenantId == tenantId, cancellationToken);
        }

        public User FindByName(string normalizedUserName, Guid? companyId, Guid? tenantId)
        {
            ThrowIfDisposed();
            return Users.FirstOrDefault(u => u.NormalizedUserName == normalizedUserName && u.CompanyId == companyId && u.TenantId == tenantId);
        }
        ...
        /// <param name="normalizedRoleName">The normalized role name.</param>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/> used to propagate notifications that the operation should be canceled.</param>
        /// <param name="companyId">The id of the id to which this role belongs to.</param>
        /// <param name="tenantId">The id of the id to which this role belongs to.</param>
        /// <returns>The role if it exists.</returns>
        protected override Task<Role> FindRoleAsync(string normalizedRoleName, Guid? companyId, Guid? tenantId, CancellationToken cancellationToken)
        {
            return Roles.SingleOrDefaultAsync(r => r.NormalizedName == normalizedRoleName && r.CompanyId == companyId && r.TenantId == tenantId, cancellationToken);
        }
        ...
        /// <param name="cancellationToken">The <see cref="CancellationToken"/> used to propagate notifications that the operation should be canceled.</param>
        /// <returns>The <see cref="Task"/> that represents the asynchronous operation.</returns>
        public async override Task RemoveFromRoleAsync(User user, string normalizedRoleName, Guid? companyId, Guid? tenantId, CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            if (user == null)
        ...
        /// <returns>A <see cref="Task{TResult}"/> containing a flag indicating if the specified user is a member of the given group. If the 
        /// user is a member of the group the returned value with be true, otherwise it will be false.</returns>
        public override async Task<bool> IsInRoleAsync(User user, string normalizedRoleName, Guid? companyId, Guid? tenantId, CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            if (user == null)
            ...
            var role = await FindRoleAsync(normalizedRoleName, companyId, tenantId, cancellationToken);
            if (role != null)
            {
                var userRole = await FindUserRoleAsync(user.Id, role.Id, cancellationToken);
                return userRole != null;
        ...
        /// The task object containing the results of the asynchronous lookup operation, the user if any associated with the specified normalized email address.
        /// </returns>
        public override Task<User> FindByEmailAsync(string normalizedEmail, Guid? companyid, Guid? tenantId, CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            return Users.FirstOrDefaultAsync(u => u.NormalizedEmail == normalizedEmail && u.CompanyId == companyid && u.TenantId == tenantId, cancellationToken);
        }
        ...
        /// The <see cref="Task"/> contains a list of users, if any, that are in the specified role. 
        /// </returns>
        public async override Task<IList<User>> GetUsersInRoleAsync(string normalizedRoleName, Guid? companyId, Guid? tenantId, CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            if (string.IsNullOrEmpty(normalizedRoleName))
